from django.urls import path

from . import views

urlpatterns = [
    path("all/", views.ListAllPropertiesAPIView.as_view(), name="all-properties"),
    path(
        "agents/", views.ListAgentPropertyAPIView.as_view(), name="agent-properties"
    ),
    # path("create/", views.create_property_api_view, name="property-create"),
    path("create/", views.PropertyCreateAPIView.as_view(), name="property-create"),
    path("update/<slug:slug>/", views.PropertyUpdateAPIView.as_view(), name="update-property"),
    path(
        "details/<slug:slug>/",
        views.PropertyDetailView.as_view(),
        name="property-details",
    ),
    # path("update/<slug:slug>/", views.update_property_api_view, name="update-property"),
    path("delete/<slug:slug>/", views.PropertyUpdateAPIView, name="delete-property"),
    path("search/", views.PropertySearchAPIView.as_view(), name="property-search"),
]